...
struct InterestRateModel {
    ...
    func calculateInterestRate(N: Int, PV: Double, PMT: Double, FV: Double, guess: Double = 0.1, tolerance: Double = 1.0e-6, maxIterations: Int = 100) -> Double? {
        var r = guess // Initial guess for the interest rate
        var iteration = 0
        
        while iteration < maxIterations {
            // Calculate the present value of the annuity formula
            let numerator = PV * pow(1 + r, Double(N)) + PMT * ((pow(1 + r, Double(N)) - 1) / r) + FV
            // Calculate the derivative of the formula with respect to r
            let derivative = Double(N) * PV * pow(1 + r, Double(N) - 1) + PMT * (Double(N) * pow(1 + r, Double(N) - 1) * r - (pow(1 + r, Double(N)) - 1)) / (r * r)
